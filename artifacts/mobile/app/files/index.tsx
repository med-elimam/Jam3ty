...
import { useListFiles, useToggleFileFavorite } from '@workspace/api-client-react';
import { getListFilesQueryKey } from '@workspace/api-client-react';
import { useQueryClient } from '@tanstack/react-query';
import { Feather } from '@expo/vector-icons';

const FILE_TYPES = ['All', 'lecture', 'td', 'tp', 'summary', 'exam', 'correction', 'book'];

function FileTypeChip({ type, active, onPress }: { type: string; active: boolean; onPress: () => void }) {
  ...
  return (
    ...
    >
      <Text style={{ fontSize: 13, fontWeight: '600', color: active ? '#fff' : colors.mutedForeground }}>{type}</Text>
    </TouchableOpacity>
  );
}
...
export default function FilesScreen() {
  ...
  const { data, isLoading, refetch, isRefetching } = useListFiles({ search: search || undefined, type: activeType === 'All' ? undefined : activeType });
  ...
  return (
    <View style={s.root}>
      <View style={s.searchRow}>
        ...
        <TextInput style={s.searchInput} placeholder="Search files…" placeholderTextColor={colors.mutedForeground} value={search} onChangeText={setSearch} />
      </View>
      <FlatList
        horizontal showsHorizontalScrollIndicator={false}
        data={FILE_TYPES} keyExtractor={(t) => t}
        contentContainerStyle={{ paddingHorizontal: 16, marginBottom: 8 }}
        renderItem={({ item }) => <FileTypeChip type={item} active={activeType === item} onPress={() => setActiveType(item)} />}
      />
      {isLoading ? (
        ...
        <FlatList
          ...
          ListEmptyComponent={<View style={s.empty}><Feather name="folder" size={48} color={colors.border} /><Text style={s.emptyText}>No files found</Text></View>}
          renderItem={({ item }: { item: any }) => (
            <View style={s.fileCard}>
              ...
              <View style={s.fileInfo}>
                ...
                <Text style={s.fileMeta}>{item.uploaderName} · {item.fileType}</Text>
...
const styles = (colors: ReturnType<typeof useColors>) =>
  StyleSheet.create({
    ...
    fileCourseName: { fontSize: 11, color: colors.navy, marginTop: 2 },
