...
import { db, clubsTable, clubMembersTable, clubJoinRequestsTable } from "@workspace/db";
...
router.get("/clubs", requireAuth, async (req, res) => {
  try {
    const clubs = await db.select().from(clubsTable).where(eq(clubsTable.status, "active")).orderBy(clubsTable.name);
