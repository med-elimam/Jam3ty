...
**Static file serving in production** (`artifacts/api-server/src/app.ts`, only when `NODE_ENV=production`): serves `public/admin` at `/admin` and `public/student` (the **mobile web export**) at `/`, each with an SPA-fallback regex registered *after* `express.static`; student's catch-all `/^(?!\/api).*/` is intentionally broad — a bad `/admin/...` path silently falls through to the student app. Deployment builds these folders: Railway's build command is `pnpm run build:all` (build API + mobile web export + admin build + `copy:builds`), so pushing source changes is enough — `public/student` is untracked/ephemeral. (A stale `public/admin` build is still tracked in git; it gets overwritten by `copy:builds` at deploy time — don't hand-edit it.)
